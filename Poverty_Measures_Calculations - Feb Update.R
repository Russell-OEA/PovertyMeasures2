...
Poverty_data <- 
  Poverty_data %>%
    ...
    mutate(Population_Threshold = if_else(PopChange_Value <= 0, "Meets or exceeds", "Does not meet")) %>%
    mutate(All_Thresholds = if_else(OBC_Poverty_Percent >= 35 & Unemp_Diff >= 0 & PopChange_Value <= 0, "Yes", "No"))
...
Poverty_Data_Final <- Poverty_data %>% select(CODE, COUSUB, Muni, County, MRI_Rank, OBC_Poverty_Percent, OBC_Poverty_Threshold, ANN_AV_UNEMP_RATE, Unemp_Diff, Unemp_Threshold, PopChange_Value, Population_Threshold, All_Thresholds)

write_csv(Poverty_Data_Final, "./Poverty_Data_Final_Feb_Update.csv")
#' 
#' ####
#' 
